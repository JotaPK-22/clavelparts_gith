...
export default function Topbar({ isSticky = true, currentView }: TopbarProps) {
  ...
  const isGarageView = currentView === 'garage'
  ...
  function handleGarageClick() {
    ...
    setView(isGarageView ? 'home' : 'garage')
  }
  ...
  return (
    <header
      className={`${isGarageView ? 'fixed top-0 left-0 right-0' : isSticky ? 'sticky top-0' : ''} z-[350] flex items-center justify-between px-4 md:px-10 border-b-2`}
      style={{
        background: isGarageView ? 'rgba(8,10,12,0.65)' : 'var(--dark2)',
        backdropFilter: isGarageView ? 'blur(12px)' : undefined,
        borderColor: isGarageView ? 'rgba(255,255,255,0.06)' : 'var(--dark3)',
        height: 78,
      }}
    >
      {/* Logo */}
      ...
      <div className="flex items-center gap-2">
        ...
        {isAuthed && (
          ...
          >
            ...
            <span className="vender-label">VENDER TU REPUESTO</span>
          </Link>
        )}

        {/* Mi Garage — bloqueado sin sesión, toggle a home cuando ya estás en garage */}
        <button
          ...
          title={isAuthed ? (isGarageView ? 'Volver al home' : 'Tu garage') : 'Iniciá sesión para ver tu garage'}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-[22px] h-[22px]">
            <path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
            <polyline points="9 22 9 12 15 12 15 22" />
          </svg>
          <span className="topbar-label">{isGarageView ? 'HOME' : 'MI GARAGE'}</span>
          {!isAuthed && (
        ...
        {authLoading ? (
          ...
          >
            ...
            <span className="topbar-label">INICIAR SESIÓN</span>
          </button>
        )}
      </div>
      ...
      <style jsx>{`
        ...
        /* Responsive: en mobile escondemos los labels de los botones para ahorrar espacio */
